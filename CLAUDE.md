...
**フェーズ3（リスト表）に着手する段階。** フェーズ1・2は完了・main にマージ済み。
...
| | |
|---|---|
| F-03 | リスト表。契約一覧を得意先・納品先・契約番号でフィルタ。明細不要の設定もここ |
| P-03 | `/periods/[id]/contracts` |

完了条件は3条件のフィルタが動き、`contract.skip_statement` の
トグルが翌月以降も継承されること（フェーズ2で洗い替えの対象外に
なっていることは検証済み）。
