...
計算自体は正しかったが、「今見ている一覧が最新の設定を反映しているか」
が画面から分からないのは実害があるため、P-06に古さの検知を追加した:

- 現在明細不要な契約IDの集合と、実際に表示中の明細書の契約IDを突き合わせ、
  重なりがあれば「⚠ 契約○○は現在明細不要ですがこの生成結果にはまだ
  含まれています。再生成してください」を明細書カードごとに表示し、
  該当行に「要再生成」タグを付ける
- 検知できるのは「除外されるはずなのにまだ残っている」方向のみ（お金を
  多く請求してしまう危険な方向）。「再度含めるべきなのに古い生成結果で
  除外されたまま」の逆方向は今回は対象外（P-03の一覧を見れば契約自体は
  必ず存在するので実害は限定的と判断）
