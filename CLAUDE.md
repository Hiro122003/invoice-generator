...
**フェーズ7（チェック機能）が完了。** フェーズ1〜6は完了・main にマージ済み。
...
### フェーズ7: F-07 発行前チェック で出来ていること

- `GET /api/periods/{id}/validate` — 4種類の観点で異常を一覧する。
  税率を問わず両方チェックする（VBA原本の`明細数の確認()`は10%シートしか
  見ていなかった。意図的な改善）
    1. **請求漏れ**（HIGH）: 請求対象の明細行があるのに、どの明細書にも
       割り当てられていない契約（`billing_line.statement_id IS NULL`）
    2. **金額0円**（MEDIUM）: 請求対象の明細行なのに `amount = 0`
    3. **期間外**（MEDIUM）: 明細のレンタル期間（`rental_start`/
       `rental_end`）が請求期間と重ならない
    4. **前月比**（INFO/MEDIUM）: 契約単位で直前の請求期間とDB上で比較し、
       新規契約・消滅契約・金額急増急減（差額1万円以上 かつ 変化率30%
       以上、両方満たしたときだけ報告。VBAに原典のない新設の閾値）を検出
- 画面: P-07 `/periods/[id]/validate`。P-03/P-06/P-08と相互リンク
- 読み取り専用。書き込みは一切しないが、複数のSELECTにまたがるチェックの
  途中で並行して生成・確定・手修正が走ると結果が一つの瞬間のスナップ
  ショットにならないため、generate/confirm/PATCH/exportと同じ
  `pg_advisory_xact_lock(period_id)` を取得してから読む（横並び確認ずみ）

**設計判断の記録（前月比の意味）**: `docs/domain-model.md`は元ネタを
VBAの`リスト表と営業データの差分()`としていたが、実際のVBAソースを
読むと「前月との比較」ではなく、リスト表と**営業部が別途持つ見込み
データ**（同じシートの別列に手で貼り付け）との突き合わせだった。この
ドメインモデルには存在しないデータソースを前提にしており、忠実に移植
すると新しい取込画面が必要になりスコープが大きく膨らむ。ユーザーに
選択肢を提示し、「本当の前月比較（直前の請求期間とDB上で比較。追加の
取込機構は不要）」を選んでもらい、その方針で実装した。**設計書に書かれた
移植元の記述を鵜呑みにせず、実際のVBAソースと突き合わせて食い違いが
あれば止めて確認する。**

**教訓**: 実データ（期間23・2025-03）で試したところ、金額0円が38件・
期間外が12件検出された。「消火器」「配送費」のような一部の品目群に
0円請求が偏っている、レンタル終了から数ヶ月後も請求が続いている行が
